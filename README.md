...
— in short: UPS and the Topology B onboarding generator are designed but not yet implemented and
must be built **before** `rc.1` (a candidate may defer proof, never existence — ADR-0012; heartbeat,
alert delivery, dyndns, and public ingress, the other four surfaces this applied to, are now
implemented — see below); T-E/R3, R4, T-C, T-D (arm64), and T-F remain unproven, must not be
claimed, and are what the RC qualification window itself exists for. See
