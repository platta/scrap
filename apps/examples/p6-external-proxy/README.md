...
`endpointslice.yaml` points at `${EXAMPLE_P6_BACKEND_ADDRESS}`, which ships in
`clusters/example/instance-config.yaml` as an RFC 5737 documentation address -- a placeholder,
never a real one, exactly like that file's `NODE_ADDRESS`. Set it to your device's real LAN address
to make this pattern actually route traffic; until then, this example demonstrates the shape of
the pattern, not a working proxy.
