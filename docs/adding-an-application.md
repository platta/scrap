...
| need a non-HTTP port (a game server, a database you expose directly, a custom TCP/UDP protocol)? | **P4 — raw TCP/UDP** | A `LoadBalancer` `Service` whose port is declared in a `reserved-ports.yaml` alongside your app's own manifests — never `hostNetwork`, never an ad hoc host port, and never a `platform/` edit ([`docs/decisions/0017`](decisions/0017-p4-port-reservation-ownership.md)). |
