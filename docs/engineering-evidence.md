...
| **T-B — Standard** | every PR | A separate from-zero cluster with identity (SSO), Grafana, and logs turned on: prove real logins work, prove an unauthenticated request is actually rejected, prove a real log line is shipped and retrievable, prove identity's account-recovery flow can't be abused. |
...
| **T-A-alert-heartbeat** | every push/PR | Prove a real, live-fired alert is genuinely delivered to a webhook receiver, and that the external heartbeat genuinely pushes only while the cluster is healthy — proven both ways, including a deliberately-unhealthy negative control. |
| **T-A-dyndns** | every push/PR | Prove a real DNS `UPDATE` genuinely repoints a hostname when the public IP changes, sends nothing when it hasn't, and fails visibly on a wrong credential — against a real, ephemeral nameserver this test stands up itself. |
| **T-A-public-ingress** | every push/PR | Prove the operator's own verification tool for public reachability is sound — its certificate-identity check genuinely passes against the real platform and genuinely fails against an unrelated endpoint. Public ingress ships no manifest, so there's no enable/disable cycle to test; this proves the *tool an operator relies on* instead. |
| **T-A-ups** | every push/PR | Prove the host-level shutdown daemon genuinely triggers on a real (simulated) power-loss event and stays silent otherwise, that the in-cluster exporter genuinely reads that state and fails visibly on misconfiguration, and that the kubelet shutdown-grace-period mechanism is genuinely armed on the host, not just configured on paper. |
| **T-A-topology-b** | every push/PR | Prove a generated separate operator repository (Topology B) genuinely bootstraps and reconciles, sourcing platform content from an independently pinned upstream — both structurally and live, with a passing negative control on the pin itself. |
