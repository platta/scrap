...
| Identity | see [`identity.md`](identity.md) | OIDC issuer + forward-auth endpoint + group claim | Whatever recovery/MFA/passkey behavior an alternative provider offers is that provider's own — verify explicitly. This is not a downgrade from what SCRAP's own Authentik configuration provides: SCRAP ships operator-mediated recovery only and neither configures nor tests passkey/WebAuthn login itself |
