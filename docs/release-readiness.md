...
| **UPS is an implemented, working capability** | README-only, no manifests | Yes — **and blocks `rc.1`**: an existence gap, not a proof gap (`docs/decisions/0012`). Dyndns closed this gap for itself — see PROVEN NOW, above — leaving UPS and public ingress |
| **Public ingress is an implemented, working capability** | Its implemented form ships no manifests **by design** (`docs/decisions/0014` — the capability is operator-edge configuration): what's missing is the executable product surface that record requires — the enablement runbook, the operator-run verification script, and CI proof of that script's own oracle. Live public exposure of a real install is a *permanent* operator-run evidence boundary (like real-domain issuance below), not part of this gap. | Yes — **and blocks `rc.1`**: an existence gap, not a proof gap (`docs/decisions/0012`; what existence means here is defined by `docs/decisions/0014`) |
