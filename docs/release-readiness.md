...
| Topology B (`docs/decisions/0009-repository-topology.md`): `bootstrap/generate-topology-b.sh` produces a minimal, ordinary Flux/Kustomize/SOPS operator repository (no copy of `platform/`, `capabilities/`, or `components/`) wired to a separately, independently pinned `scrap-platform` `GitRepository` with every platform-tier `Kustomization`'s `sourceRef` genuinely swapped; a generated repository actually bootstraps and reconciles end to end from a from-zero host, proven both structurally and live (the running Kustomization's own `sourceRef.name` reads `scrap-platform`, the pinned commit genuinely resolves, and real upstream content — the wildcard `Certificate` — genuinely issues), with a passing negative control (a wrong pinned commit fails visibly and recovers cleanly once corrected) | `tests/profiles/t-a-topology-b.sh` |
| Satellite CORE observability topology (`docs/decisions/0018-observability-topology.md`): `platform/observability-satellite/`'s values-delta (retention, storage, `remoteWrite`) genuinely reaches the Prometheus CR; a real remote-write delivery is independently confirmed via an ephemeral, non-SCRAP Prometheus receiver's own query API (never inferred from the satellite's own reported state); local baseline-rule evaluation and Alertmanager delivery are unchanged and genuinely keep working during a live-induced remote-write outage; a bad remote-write credential and an induced path/destination outage both fail visibly through the new F3 telemetry-path-health rule (`SatelliteRemoteWriteFailing`/`SatelliteRemoteWriteStale`); standalone is unaffected until this topology is explicitly selected, and reverting is clean — see `platform/observability-satellite/README.md` for the honest limits (F2 whole-node death, WAL buffering bounds, and any ARM64/small-node claim are explicitly out of this evidence's scope) | `tests/profiles/t-a-observability-satellite.sh` |
