...
| **UPS, public-ingress are implemented, working capabilities** | README-only, no manifests, in every case | Yes — **and blocks `rc.1`**: an existence gap, not a proof gap (`docs/decisions/0012`). Dyndns closed this gap — see PROVEN NOW, above — leaving these two |
