...
| **UPS is an implemented, working capability** | README-only, no manifests | Yes — **and blocks `rc.1`**: an existence gap, not a proof gap (`docs/decisions/0012`). Dyndns and public ingress both closed this gap for themselves — see PROVEN NOW, above — leaving UPS as the last one |
