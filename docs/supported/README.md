...
| Logs | Centralized, searchable pod logs | ~300 MB RAM | **Designed, not yet implemented** — README only, no manifests | `capabilities/logs/` |
| Alert delivery | Alerts that actually reach you | a reachable SMTP/ntfy/webhook receiver | **Designed, not yet implemented** — Alertmanager is deliberately left at its default `null` receiver; alert evaluation and a visible surface are CORE and proven, a configured delivery receiver is not built | `capabilities/README.md` |
...
| External heartbeat | Told when the *cluster itself* is down | a free third-party account | **Designed, not yet implemented** — README only, no manifests | `capabilities/heartbeat/` |
