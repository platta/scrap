...
| **Standard** | ~6–8 GB | + Grafana, Logs (both implemented) | Comfortable headroom for a handful of small applications. Grafana adds ~250–400 MB RAM; Logs (Loki + Alloy) adds ~200–700 MB RAM — see `capabilities/logs/README.md`. Alert delivery is designed but not yet implemented — see `capabilities/README.md`'s Status column — so it adds nothing to this tier's real footprint today |
