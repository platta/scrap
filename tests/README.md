...
| [`profiles/`](profiles/) | Dynamic, cluster-backed acceptance profiles (T-A minimal through T-F upgrade) | **T-A implemented**; **T-B implemented** for identity + P2/P3 + Grafana + logs + the recovery-flow-abuse adversarial check (the originally-scoped T-B definition is now fully covered); **T-A-public-tls implemented**; **T-A-offsite-backup implemented** (proves artifact placement, not R3 — see `docs/release-readiness.md`); T-C, T-D, T-E, T-F not yet |
