...
| **T-C — Connected** | nightly | ACME issuance via DNS-01 against a real test zone; heartbeat delivery; an integrated nightly run of the off-site-backup path alongside them | Not yet implemented — off-site backup itself now has its own dedicated live acceptance surface (T-A-offsite-backup, above); T-C's remaining, unimplemented value is the nightly integration with DNS-01 and heartbeat (heartbeat doesn't exist yet either) |
